        ...
        if(isBack){
            ...
            VStack(alignment:.center,spacing:0){
                ...
                VStack(){
                    ...
                    Slider(value: $frequencySelected, in: -1000...1000, step: 5){ _ in
                        changePitch()
                    }.disabled(isDone ? false : true).padding(.top,10)
                  
                    Text(String(frequencySelected)).foregroundColor(Color(.white)).font(.callout).padding(.bottom,20)
                    
                        .multilineTextAlignment(.center).opacity(isDone ? 1 : 0.5)
                    
                    Text("The key will be default based on\nyour vocal range test results").foregroundColor(Color(.white)).font(.callout)
                        .multilineTextAlignment(.center).opacity(isDone ? 1 : 0.5)
                    HStack(){
                ...
                HStack(spacing:10){
                    ...
                    }.opacity(isDone ? 1 : 0).disabled(isDone ? false : true)
            ...
            }.padding(30).padding(.top,60).padding(.bottom,130).frame(maxWidth:.infinity, maxHeight:.infinity).background(Color("Blue")).ignoresSafeArea().onAppear{
                setupAudioEngine()
            }
        }
    }
